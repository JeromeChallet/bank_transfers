...
final class FinalResultTest extends TestCase 
{
    private $expected_return = [
        "filename"=>"data_sample.csv",// filename is the filename from the path
        "failure_code"=>"100",//failure code is the second field in the header line
        "failure_message"=>"All systems go",//failure message is the third field in the header line
        "records"=>[
    ...
    protected function setUp() :void{
        $this->f = new FinalResult();  // Create a new FinalResult object
    }

    public function testReturnsTheCorrectHash(): void 
    {
        //given-- Arrange
        //$f = new FinalResult(); // Create a new FinalResult object

        //when -- Act
        $res = $this->f->results('tests/support/data_sample.csv'); // Read the data from the sample file
        unset($res["document"]); // Remove the document from the results

        //then -- Assert
        $this->assertEquals($res, $this->expected_return); // Assert that the results are the same as the expected results
    }
